...
def submit():
	...
	if form.campus.data != "None":
		classes = [x for x in classesIn[form.campus.data] if x["time"] > time and x["day"] == day and (form.building.data == [] or x["building"] in form.building.data)]
	print(form.building.data)
	classBuildings = [x["building"] for x in classes]
	form.building.choices = [(x["code"], x["name"]) for x in dataIn["buildings"] if x["campus"] == form.campus.data and x["code"] in classBuildings]
